	...
	function changeflash(i) {	
		currentindex=i;
		for (j=1;j<=2;j++){//此处的2代表你想要添加的幻灯片的数量与下面的2相呼应
			if (j==i){
	...
	function startAm(){
		timerID = setInterval("timer_tick()",2000);//2000代表间隔时间设置
	}
	function stopAm(){
		clearInterval(timerID);
	}
	function timer_tick() {
		currentindex = currentindex >=2 ? 1 : currentindex + 1;//此处的2代表幻灯片循环遍历的次数

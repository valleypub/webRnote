...
import com.kang.webRnote.service.UserService;
...
public class UserServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        log.info("actionName is"+actionName);
        // 判断用户行为 调用对应方法
        if ("login".equals(actionName)) {
            ...
            log.info("未匹配任何的");
//            //这个是为了直接在url中输入http://localhost:8080/note/user时能跳转到用户登录login.jsp页面
//            response.sendRedirect("login.jsp");
            //为了适配免登陆功能：应该直接交给indexServlet，如果已经登录就输入user也直接进入index页面，否则会被拦截进入login登陆界面
    ...
    private void checkNick(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // 4. 通过字符输出流将结果响应给前台的ajax的回调函数
        response.getWriter().write(code + ""); //整形+""变成字符串。
        // 5. 关闭资源
    ...
    private void userLogin(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (resultInfo.getCode() == 1) { // 如果成功
            // 将用户信息设置到session作用域中，后面的判断用户是否登录状态会用到

...
import com.kang.webRnote.po.Note;
import com.kang.webRnote.vo.NoteVo;
...
public class NoteDao {
    ...
    public int addOrUpdate(Note note) {
        ...
        if (note.getNoteId() == null) { // 添加操作
            ...
            //只有第一次添加的时候写经纬度，后面修改只修内容不因该修改第一次编写所在地
    ...
    public long findNoteCount(Integer userId, String title, String date, String typeId) {
        // 1. 定义SQL语句，因为tb_note没有userid，所以需要通过外联的方式查
...
//
    ...
    public List<NoteVo> findNoteCountByType(Integer userId) {
        ...
        String sql = "SELECT t.typeId, typeName groupName, count(noteId) noteCount, n.typeId FROM tb_note n " +
                ...
                " GROUP BY n.typeId ORDER BY COUNT(noteId) DESC ";
    ...
    public Note findNoteById(String noteId) {
        // 定义 sql 语句
        String sql = "select n.noteId,n.title,n.content,n.pubTime,n.typeId,t.typeName,t.typeId from tb_note n " +
                "inner join tb_note_type t on n.typeId=t.typeId where n.noteId = ?";
    ...
    public List<Note> queryNoteList(Integer userId) {
        // 定义 sql 语句
        String sql = "select lon, lat, n.typeId " +

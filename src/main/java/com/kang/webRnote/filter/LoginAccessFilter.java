package com.kang.webRnote.filter;
...
public class LoginAccessFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse
        ...
        // 1. 指定页面，放行（用户无需登录的即可访问的页面；例：登录页面login.jsp、注册页面register.jsp等）
        if (path.contains("/login.jsp")) {
            log.info("进入login.jsp");
            //放行
        ...
        if (cookies != null && cookies.length > 0) {
            ...
            for (Cookie cookie : cookies) {
                if ("user".equals(cookie.getName())) {
                    ...
                    //这一次两者都不会再拦截了，为什么，这不也是请求吗？？万一这里面有中文？？
                   RequestDispatcher rd =  request.getRequestDispatcher(url);
                    String mm = request.getParameter("actionName");
                   rd.forward(request, response);

                    log.info("进入一次cookies");
                    // 8. return
                    return;
                }
            }
